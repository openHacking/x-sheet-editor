...
class StyleWrapUp extends StyleFormat {
  ...
  wrapUpStyle(name, value, inside) {
    ...
    if (SheetUtils.isUnDef(checking)) {
      selection.wrap({ container, inside });
    } else {
      selection.split({ checking, container, inside });
    }
  }

  toggleStyle(name, value, inside) {
    let cssKeying = name;
    let container = h('span').css(cssKeying, value);
    let selection = new Selection({
      root: this,
    ...
    if (SheetUtils.isUnDef(checking)) {
      ...
      selection.split({ checking, inside });
    }
  }
}
...
class StyleEdit extends StyleWrapUp {
  ...
  underLine() {
    this.customStyle = true;
    this.toggleStyle('text-decoration', 'underline', true);
  }

  strikeLine() {
    this.customStyle = true;
    this.toggleStyle('text-decoration', 'line-through', true);
  }
