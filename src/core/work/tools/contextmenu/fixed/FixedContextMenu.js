...
class FixedContextMenu extends ELContextMenu {
  ...
  constructor(options = {}) {
    ...
    Event.bind(this.row, Constant.SYSTEM_EVENT_TYPE.CLICK, () => {
      const { options } = this;
      options.onUpdate('ROW');
      this.close();
    });
    Event.bind(this.row1, Constant.SYSTEM_EVENT_TYPE.CLICK, () => {
      const { options } = this;
      options.onUpdate('ROW1');
      this.close();
    });
    Event.bind(this.row2, Constant.SYSTEM_EVENT_TYPE.CLICK, () => {
      const { options } = this;
      options.onUpdate('ROW2');
      this.close();
    });
    Event.bind(this.col, Constant.SYSTEM_EVENT_TYPE.CLICK, () => {
      const { options } = this;
      options.onUpdate('COL');
      this.close();
    });
    Event.bind(this.col1, Constant.SYSTEM_EVENT_TYPE.CLICK, () => {
      const { options } = this;
      options.onUpdate('COL1');
      this.close();
    });
    Event.bind(this.col2, Constant.SYSTEM_EVENT_TYPE.CLICK, () => {
      const { options } = this;
